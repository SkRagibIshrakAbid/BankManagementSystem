...
include('../db/db.php');
include('../control/deleteusermer.php');
?>
...
<body style="background-image: linear-gradient(to right, rgba(255,0,0,0), rgb(87, 137, 194))">
    ...
    <div class='eReg'>
        <h2 class='top-title'>Merchant delete</h2> 
        </div>
...
$userQuery=$connection->ShowAll($conobj,"merchant",$_COOKIE['deleteck']);

if ($userQuery->num_rows > 0) {
    echo "<table class='empsearch'><tr><th>id</th><th>name</th><th>username</th><th>password</th><th>license</th><th>mobile</th><th>address</th><th>balance</th></tr>";
    
    while($row = $userQuery->fetch_assoc()) {
      echo "<tr><td>".$row["id"]."</td><td>".$row["name"]."</td><td>".$row["username"]."</td><td>".$row["password"]."</td><td>".$row["license"]."</td><td>".$row["mobile"]."</td><td>".$row["address"]."</td><td>".$row["balance"]."</td></tr>";
    }
    echo "</table>";
  } else {
    echo "0 results";
...
?>
<h4 class='e1'>Do you want to delete this Merchant?</h4><br>
<form action="" method = "post">
<input class='reg-button button1' name='delete' type='submit' value='Confirm delete'></div>
<a href="adminmerchantall.php">Go back</a>
</form>
